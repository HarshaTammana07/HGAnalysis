...
Paste these two cells into the reusable `nb_control_audit_writer` notebook.

This version follows the new DartsSrv control model:

- Bronze uses one active `taskconfig` row per site for `ConfigId = 25`.
- Silver uses one active `taskconfig` row for `ConfigId = 26`.
- Gold uses one active `taskconfig` row for `ConfigId = 27`.
