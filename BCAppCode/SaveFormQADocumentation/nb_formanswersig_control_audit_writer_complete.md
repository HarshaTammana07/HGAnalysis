...
- Gold uses one active `taskconfig` row for `ConfigId = 33`.
...
if mode == "START_LAYER_RUNS":
...
elif mode == "FINALIZE_FORMANSWERSIG_SUCCESS":
    ...
    if not br or not sl or not gl:
        raise Exception("Missing BR/SL/GL context for FINALIZE_FORMANSWERSIG_SUCCESS")
    if not p_ingest_run_id:
        raise Exception("p_ingest_run_id is required for FINALIZE_FORMANSWERSIG_SUCCESS")
...
elif mode == "FINALIZE_FORMANSWERSIG_FAILURE":
    br = get_context_for_target(ctx, "BR")
    sl = get_context_for_target(ctx, "SL")
    gl = get_context_for_target(ctx, "GL")

    if not br or not sl or not gl:
        raise Exception("Missing BR/SL/GL context for FINALIZE_FORMANSWERSIG_FAILURE")
    if not p_ingest_run_id:
        raise Exception("p_ingest_run_id is required for FINALIZE_FORMANSWERSIG_FAILURE")
    ...
    failure_message = (
        p_error_message
        or "FormAnswerSignatures pipeline failed. Check the failed parent/child activity run details."
    )
