...
namespace BHG_DR_LIB
{
    ...
    {
        public Models.RCodes SaveBills(DataTable tbl, string sc, DateTime wrkdt, bool yearly, Models.BHG_DRContext db)
        {
                ...
                if (db == null) { db = new Models.BHG_DRContext(); }
                {
                    try
                    {
                        ...
                        if (yearly)
                        {
                            ...
                            && x.BillDate.Value.Year == wrkdt.Year
                        ...
                        {
                            bills = db.TblBills.Where(x => x.SiteCode == sc
                            && x.BillDate >= wrkdt.AddMonths(-1)
                            && x.BillDate <= wrkdt.AddDays(1)).ToList();
                        }
                        ...
                        foreach (DataRow r in tbl.Rows)
                        {
                            ...
                            int bcltid = int.Parse(r["billcltid"].ToString());
                            int myrcs = int.Parse(r["RowChkSum"].ToString());
                            //if (bcltid == null) { bcltid = 0; }
                            //if (AllNewRows)
                            //{
                            //    NewRow = true;
                            ...
                            //else
                            //{
                            bill = bills.Where(x => x.BillId == billid
                                && x.BillCltid == bcltid).FirstOrDefault();
                            if (bill == null)
                            {
                                NewRow = true;
                                bill = new Models.TblBills
                            ...
                            if ((bill.RowChkSum != myrcs) || (NewRow))
                            {
                                ...
                                bill.BillReason = r["billreason"].ToString();
                                if (r["billreceiptnum"].ToString().Length > 0) { bill.BillReceiptNum = int.Parse(r["billreceiptnum"].ToString()); }
