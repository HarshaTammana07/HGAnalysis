...
reference_site_success_table = "bhg_bronze.P1Forms.br_p1_forms_site_success"
...
def ensure_reference_site_success_table():
    spark.sql(f"""
        CREATE TABLE IF NOT EXISTS {reference_site_success_table} (
            Method STRING,
            SiteCode STRING,
...
def reference_site_success_markers(method_name):
    if not p_ingest_run_id:
        return set()
    try:
        marker_df = (
            spark.table(reference_site_success_table)
            .where(F.col("IngestRunId") == F.lit(p_ingest_run_id))
            .where(F.lower(F.col("Method")) == F.lit(str(method_name).lower()))
            .select("SiteCode", "SourceDatabase")
            .distinct()
        )
    except Exception:
        return set()
...
def site_results_from_markers(br_layer, method_name, br_result):
    method_layer = filter_layer_by_method(br_layer, method_name)
    if not method_layer:
        return None

    markers = reference_site_success_markers(method_name)
    if not markers:
        return None
    ...
    for task in layer_tasks(method_layer):
        ...
        if (site_code, database_name) in markers or (site_code, None) in markers:
            ...
            item.update({
                "status": "FAILED",
                "failed_stage": "BR",
                "error_message": result_error_message(br_result) or f"{method_name} Bronze site failed before writing success marker."
            })
        else:
            item["status"] = "SUCCESS"
        results.append(item)
...
def build_terminal_method_results(br_layer, sl_layer, bronze_json, silver_json):
    ...
    for method in methods:
        ...
        site_results = (
            ...
            or site_results_from_markers(br_layer, method, br_result)
        )
        layer_statuses = {
...
if mode == "START_LAYER_RUNS":
    ensure_reference_site_success_table()
