...
def with_delta_retry(action, label, max_attempts=10, base_sleep_seconds=2, max_sleep_seconds=30):
    last_error = None
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            if attempt >= max_attempts or not is_delta_concurrency_error(exc):
                raise
            sleep_seconds = min(max_sleep_seconds, base_sleep_seconds * (2 ** (attempt - 1))) + random.uniform(0, 3)
            print(f"{label}: concurrent Delta write detected on attempt {attempt}/{max_attempts}; retrying in {sleep_seconds:.1f}s")
...
def delta_upsert(table_name, df, keys):
    def action():
        ...
        updates = {c: f"s.{c}" for c in aligned_df.columns if c not in keys}
        inserts = {c: f"s.{c}" for c in aligned_df.columns}
        return (
            DeltaTable.forName(spark, table_name).alias("t")
            .merge(aligned_df.alias("s"), condition)
