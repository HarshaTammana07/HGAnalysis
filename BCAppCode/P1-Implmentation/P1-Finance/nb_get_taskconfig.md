...
output_columns = [
    str(x).strip()
    for x in parse_json_list(p_output_columns_json, "p_output_columns_json")
    if str(x).strip()
]
...
if output_columns:
    ...
    missing_columns = [column for column in output_columns if column not in available_columns]
    if missing_columns:
        raise Exception(f"Invalid p_output_columns_json columns: {missing_columns}")
